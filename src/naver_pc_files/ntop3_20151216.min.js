...
break}},selectCategory:function(e,f,a){var b=this;if(!f){f=(!global.isSupportedFlicking)?this.onLoadPage:this.Flicking.loadBothCategory}var d=this.cacheData[e];if(d){return f(e,d,a)}var c="/nvtheme_body2?theme="+e;
$Ajax(c,{timeout:3,method:"get",onload:function(h){var g=h.text();b.cacheData[e]=g;f(e,g,a)},onerror:function(){JEagleEyeClient.sendError("[THEMECAST] ajax fail(onerror) - url : "+c)},ontimeout:function(){JEagleEyeClient.sendError("[THEMECAST] ajax fail(ontimeout) - url : "+c)
}}).request()},showConfig:function(){var a=this;var c=$Cookie().get("NM_THEMECAST_USER");var d=c?c:"tcc_non";var b="/include/themecast/menu_config.2015.html";if(a.configLayer.child().length>0){a.configLayer.show();
$("config_"+d).checked=true;$ElementList("#theme_config .radio-mark").removeClass("radio-checked");$Element($$.getSingle("#theme_config .csl_"+d+" .radio-mark")).addClass("radio-checked")}else{$Ajax(b,{timeout:3,onload:function(e){a.configLayer.html(e.text());
a.configLayer.show();$("config_"+d).checked=true;$ElementList("#theme_config .radio-mark").removeClass("radio-checked");$Element($$.getSingle("#theme_config .csl_"+d+" .radio-mark")).addClass("radio-checked")
},onerror:function(){JEagleEyeClient.sendError("[THEMECAST] ajax fail(onerror) - url : "+b)},ontimeout:function(){JEagleEyeClient.sendError("[THEMECAST] ajax fail(ontimeout) - url : "+b)}}).request()}},hideConfig:function(){this.configLayer.hide()
},setConfigCookie:function(){var a=$$.getSingle("#theme_config input[checked=true]").id.replace("config_","");if(a&&a!="tcc_non"){$Cookie().remove("NM_THEMECAST_LAST");$Cookie().set("NM_THEMECAST_USER",a,3650);
this.selectCategory(a)}else{if(a=="tcc_non"){$Cookie().remove("NM_THEMECAST_USER")}}this.hideConfig()},EventManager:{init:function(){$Element("themecast_cate").delegate("click","a",this.onclickCategory);
$Element("themecast_contents").delegate("click",".cpg a.pre",this.onclickPrev);$Element("themecast_contents").delegate("click",".cpg a.next",this.onclickNext);$Element("themecast_contents").delegate("click",".tc_bottom .tc_set",this.onclickConfig);
$Element("themecast_contents").delegate("click",".area_book .opg a",this.onclickBookMoveBtn);$Element("themecast_contents").delegate("click","a:not([href^=#])",this.onclickContentsLink);$Element("theme_config").delegate("click",".cast_set_lst li",this.onclickRadio);
$Element(document).attach("click",this.onclickDocument)},onclickCategory:function(b){b.stop();var c=$Element(b.element);if(c.hasClass("on")){return}var d=$Element(b.element).attr("data-code");TC.selectCategory(d);
var a="tct."+d.replace("tcc_","");clickcr(b.element,a,"","",b._event);TC.UI.showFlickingNoti()},onclickPrev:function(c){c.stop();var d=TC.cateArr.indexOf($Element($$.getSingle("#themecast_cate .on")).attr("data-code"));
...
}}).request();a.Layer.leave("opc_layer_unsubs")},EventManager:{init:function(){$Element("themecast_contents").delegate("click",".opc_cate a[data-code]",this.onclickCategory);$Element("themecast_contents").delegate("click",".opc_w .opg a.pre",this.onclickPrev);
$Element("themecast_contents").delegate("click",".opc_w .opg a.next",this.onclickNext);$Element("themecast_contents").delegate("click",".opc_w .rd",this.onclickSubscribe);$Element("themecast_contents").delegate("click",".opc_w .del",this.onclickUnSubscribe)
...
}},Layer:{Logout:{html:'<div id="opc_layer_logout" class="ly_oc ly_oc2"><p>로그인 후 MY캐스트를<br>이용해보세요.</p><a href="#" title="닫기" class="close" onclick="TC.Opencast.Layer.leave(\'opc_layer_logout\');return false;">닫기</a><span class="arrow"></span></div>',show:function(){TC.Opencast.Layer.leave("opc_layer_logout");
